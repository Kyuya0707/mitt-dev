"use client";

import { useEffect, useState } from "react";

type BestViewPayoutRow = {
  id: string;
  createdAt: string;
  updatedAt: string;
  recipientUserId: string;
  recipientType: string;
  amount: number;
  currency: string;
  status: string;
  stripeAccountId: string | null;
  stripeTransferId: string | null;
  transferredAt: string | null;
  failureReason: string | null;
  recipientUser: {
    id: string;
    email: string;
    username: string | null;
    stripeAccountId: string | null;
    stripeConnectPayoutsEnabled: boolean;
    stripeConnectDetailsSubmitted: boolean;
  };
  revenueShare: {
    id: string;
    purchaseId: string;
    questionId: string;
    answerId: string;
    buyerId: string;
    questionOwnerId: string;
    answerOwnerId: string;
    grossAmount: number;
    questionOwnerAmount: number;
    answerOwnerAmount: number;
    platformFeeAmount: number;
    currency: string;
    status: string;
  };
};

function getStatusLabel(status: string) {
  switch (status) {
    case "pending":
      return "未送金";
    case "processing":
      return "処理中";
    case "paid":
      return "送金済み";
    case "failed":
      return "失敗";
    default:
      return status;
  }
}

function getRecipientTypeLabel(recipientType: string) {
  switch (recipientType) {
    case "question_owner":
      return "質問者";
    case "answer_owner":
      return "旧仕様の回答者分";
    default:
      return recipientType;
  }
}

function getConnectStatusText(value: boolean) {
  return value ? "完了" : "未完";
}

function shortenStripeAccountId(value: string | null) {
  if (!value) {
    return "未設定";
  }

  if (value.length <= 14) {
    return value;
  }

  return `${value.slice(0, 8)}...${value.slice(-4)}`;
}

function formatYen(value: number) {
  return `${value.toLocaleString("ja-JP")}円`;
}

export default function BestViewPayoutsTable() {
  const [payouts, setPayouts] = useState<BestViewPayoutRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const [processingId, setProcessingId] = useState<string | null>(null);

  const loadPayouts = async () => {
    setLoading(true);
    setError("");

    try {
      const res = await fetch("/api/admin/best-view-payouts", {
        cache: "no-store",
      });
      const data = (await res.json().catch(() => ({}))) as {
        payouts?: BestViewPayoutRow[];
        error?: string;
      };

      if (!res.ok) {
        setError(data.error || "BEST閲覧料 Payout 一覧の取得に失敗しました。");
        setPayouts([]);
        return;
      }

      setPayouts(data.payouts ?? []);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    void loadPayouts();
  }, []);

  const handleTransfer = async (payoutId: string) => {
    setProcessingId(payoutId);
    setMessage("");
    setError("");

    try {
      const res = await fetch(`/api/admin/best-view-payouts/${payoutId}/transfer`, {
        method: "POST",
      });
      const data = (await res.json().catch(() => ({}))) as {
        error?: string;
      };

      if (!res.ok) {
        setError(data.error || "送金に失敗しました。");
        await loadPayouts();
        return;
      }

      setMessage("送金が完了しました");
      await loadPayouts();
    } finally {
      setProcessingId(null);
    }
  };

  if (loading) {
    return <p className="text-sm text-gray-500">読み込み中...</p>;
  }

  return (
    <div>
      {message && (
        <div className="mb-4 rounded border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
          {message}
        </div>
      )}

      {error && (
        <div className="mb-4 rounded border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          {error}
        </div>
      )}

      {payouts.length === 0 ? (
        <p className="text-sm text-gray-500">未処理の BEST閲覧料 Payout はありません。</p>
      ) : (
        <div className="space-y-4">
          {payouts.map((payout) => {
            const displayName =
              payout.recipientUser.username ||
              payout.recipientUser.email ||
              payout.recipientUser.id;
            const destinationStripeAccountId =
              payout.stripeAccountId ?? payout.recipientUser.stripeAccountId;
            const canTransfer =
              Boolean(destinationStripeAccountId) &&
              payout.recipientUser.stripeConnectPayoutsEnabled &&
              payout.recipientUser.stripeConnectDetailsSubmitted &&
              payout.status !== "processing" &&
              payout.status !== "paid" &&
              !payout.stripeTransferId;

            return (
              <div
                key={payout.id}
                className="rounded border border-gray-200 bg-white p-4 shadow-sm"
              >
                <div className="grid gap-2 text-sm text-gray-800 md:grid-cols-2">
                  <p>
                    <span className="font-semibold">作成日:</span>{" "}
                    {new Date(payout.createdAt).toLocaleString("ja-JP")}
                  </p>
                  <p>
                    <span className="font-semibold">status:</span>{" "}
                    {getStatusLabel(payout.status)}
                  </p>
                  <p>
                    <span className="font-semibold">受取種別:</span>{" "}
                    {getRecipientTypeLabel(payout.recipientType)}
                  </p>
                  <p>
                    <span className="font-semibold">受取ユーザー:</span> {displayName}
                  </p>
                  <p>
                    <span className="font-semibold">email:</span>{" "}
                    {payout.recipientUser.email}
                  </p>
                  <p>
                    <span className="font-semibold">金額:</span>{" "}
                    {formatYen(payout.amount)}
                  </p>
                  <p>
                    <span className="font-semibold">Connect状態:</span>{" "}
                    detailsSubmitted={getConnectStatusText(
                      payout.recipientUser.stripeConnectDetailsSubmitted
                    )}{" "}
                    / payoutsEnabled={getConnectStatusText(
                      payout.recipientUser.stripeConnectPayoutsEnabled
                    )}
                  </p>
                  <p>
                    <span className="font-semibold">Payout stripeAccountId:</span>{" "}
                    {shortenStripeAccountId(payout.stripeAccountId)}
                  </p>
                  <p>
                    <span className="font-semibold">User stripeAccountId:</span>{" "}
                    {shortenStripeAccountId(payout.recipientUser.stripeAccountId)}
                  </p>
                  <p>
                    <span className="font-semibold">purchaseId:</span>{" "}
                    {payout.revenueShare.purchaseId}
                  </p>
                  <p>
                    <span className="font-semibold">questionId:</span>{" "}
                    {payout.revenueShare.questionId}
                  </p>
                  <p>
                    <span className="font-semibold">answerId:</span>{" "}
                    {payout.revenueShare.answerId}
                  </p>
                </div>

                <div className="mt-3 grid gap-2 rounded bg-gray-50 p-3 text-sm text-gray-800 md:grid-cols-4">
                  <p>
                    <span className="font-semibold">総額:</span>{" "}
                    {formatYen(payout.revenueShare.grossAmount)}
                  </p>
                  <p>
                    <span className="font-semibold">質問者分:</span>{" "}
                    {formatYen(payout.revenueShare.questionOwnerAmount)}
                  </p>
                  <p>
                    <span className="font-semibold">手数料:</span>{" "}
                    {formatYen(payout.revenueShare.platformFeeAmount)}
                  </p>
                </div>

                {payout.failureReason && (
                  <p className="mt-2 text-sm text-red-700">
                    <span className="font-semibold">失敗理由:</span>{" "}
                    {payout.failureReason}
                  </p>
                )}

                <div className="mt-4">
                  <button
                    type="button"
                    onClick={() => handleTransfer(payout.id)}
                    disabled={!canTransfer || processingId === payout.id}
                    className="rounded bg-gray-900 px-4 py-2 text-sm text-white disabled:opacity-50"
                  >
                    {processingId === payout.id ? "送金中..." : "送金する"}
                  </button>
                  {!canTransfer &&
                    payout.status !== "processing" &&
                    payout.status !== "paid" && (
                      <p className="mt-2 text-xs text-gray-500">
                        送金先 account または Connect 状態が未完了のため送金できません。
                      </p>
                    )}
                  {payout.status === "processing" && (
                    <p className="mt-2 text-xs text-gray-500">
                      現在この BEST閲覧料 Payout は処理中です。
                    </p>
                  )}
                </div>
              </div>
            );
          })}
        </div>
      )}
    </div>
  );
}
